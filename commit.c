...
int commit_parse(const void *data, size_t len, Commit *commit_out) {
    ...
    if (line_len + 1 > sizeof("tree ") + HASH_HEX_SIZE) return -1;
    char line_buf[128];
    memcpy(line_buf, p, line_len);
    line_buf[line_len] = '\0';

    // "tree <hex>\n"
    if (sscanf(line_buf, "tree %64s", hex) != 1) return -1;
    if (hex_to_hash(hex, &commit_out->tree) != 0) return -1;
    p = line_end + 1;

    // optional "parent <hex>\n"
    if (p < end && strncmp(p, "parent ", 7) == 0) {
        line_end = memchr(p, '\n', (size_t)(end - p));
        if (!line_end) return -1;
        ...
        memcpy(line_buf, p, line_len);
        line_buf[line_len] = '\0';
        if (sscanf(line_buf, "parent %64s", hex) != 1) return -1;
