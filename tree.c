...
int tree_serialize(const Tree *tree, void **data_out, size_t *len_out) {
    ...
    for (int i = 0; i < sorted_tree.count; i++) {
        ...
        if ((size_t)written + 1 > (max_size - offset)) {
            free(buffer);
            return -1;
        }
...
static int find_subdir(char subdirs[][256], int count, const char *name) {
    for (int i = 0; i < count; i++) {
        if (strcmp(subdirs[i], name) == 0) return i;
    }
    return -1;
}
...
static int write_tree_level(const Index *index, const char *prefix, ObjectID *id_out) {
    ...
    for (int i = 0; i < index->count; i++) {
        ...
        if (!starts_with_prefix(path, prefix)) continue;
        ...
        if (!slash) {
            ...
            size_t seg_len = (size_t)(slash - rel);
            if (seg_len == 0 || seg_len >= sizeof(subdirs[0])) return -1;

            char dirname[256];
            memcpy(dirname, rel, seg_len);
            dirname[seg_len] = '\0';
    ...
    for (int i = 0; i < subdir_count; i++) {
        ...
        entry->mode = MODE_DIR;
        entry->hash = child_id;
        size_t dir_name_len = strnlen(subdirs[i], sizeof(entry->name) - 1);
        memcpy(entry->name, subdirs[i], dir_name_len);
